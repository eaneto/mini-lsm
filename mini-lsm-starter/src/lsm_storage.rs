...
impl LsmStorageInner {
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        ...
        if self.has_reached_target_sst_size() {
    ...
    pub fn scan(
        ...
        let mut iterators = Vec::with_capacity(snapshot.imm_memtables.len() + 1);

        iterators.push(Box::new(snapshot.memtable.scan(lower, upper)));

        for memtable in snapshot.imm_memtables.iter() {
            iterators.push(Box::new(memtable.scan(lower, upper)));
        }
    ...
    fn has_reached_target_sst_size(&self) -> bool {
